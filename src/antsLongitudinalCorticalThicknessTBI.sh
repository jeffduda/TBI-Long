...
if [[ $# -lt 3 ]] ; then
  ...
  while getopts "a:b:c:d:e:f:g:h:j:k:l:m:n:o:p:q:r:s:t:v:w:x:z:" OPT
    do
      case $OPT in
          a)
       ...
       ;;
          s) #steal this letter for subject exclusion mask
       EXCLUSION_MASK=$OPTARG
       ;;
...
if [[ ${#ANATOMICAL_IMAGES[@]} -eq ${NUMBER_OF_MODALITIES} ]];
  then
    ...
    BASENAME_ID=`basename ${ANATOMICAL_IMAGES[0]} .nii.gz`
    ...
    NONLONGITUDINAL_OUTPUT_DIRECTORY=`dirname $OUTPUT_PREFIX`
    NONLONGITUDINAL_OUTPUT_DIRECTORY=${NONLONGITUDINAL_OUTPUT_DIRECTORY}/${BASENAME_ID}_0/${BASENAME_ID}
    echo $NONLONGITUDINAL_OUTPUT_DIRECTORY
    ...
    #      -g 1 \
    #      -o ${NONLONGITUDINAL_OUTPUT_DIRECTORY}
    #  else
    #    logCmd ${ANTSPATH}/antsCorticalThickness.sh \
    #      -d ${DIMENSION} \
    #      -q ${RUN_FAST_ANTSCT_TO_GROUP_TEMPLATE} \
    ...
    cp ${ANATOMICAL_IMAGES[0]} ${SINGLE_SUBJECT_TEMPLATE}
    #for (( j=0; j < $NUMBER_OF_MODALITIES; j++ ))
    #  do
    #    cp ${ANATOMICAL_IMAGES[$j] ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}T_template{$j}.nii.gz
    #  done
...
if [[ ! -d ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE} ]]; then
  logCmd mkdir -p ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}
fi
...
if [[ -f ${EXCLUSION_MASK} ]]; then
  ...
  echo "  -------------------------------------"
  echo "   Creating exclusion mask in SST space"
  echo "  -------------------------------------"

  EXCLUSION_SST=${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}/ExclusionMaskSST.nii.gz

  if [[ ${#ANATOMICAL_IMAGES[@]} -eq ${NUMBER_OF_MODALITIES} ]]; then
    logCmd cp ${EXCLUSION_MASK} ${EXCLUSION_SST}
  else
    if [[ ! -f ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}/ExclusionToSST1Warp.nii.gz ]]; then
      logCmd ${ANTSPATH}/antsRegistrationSyN.sh \
        -d ${DIMENSION} \
        -o ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}/ExclusionToSST \
        -m ${ANATOMICAL_IMAGES[0]} \
        -f ${SINGLE_SUBJECT_TEMPLATE}
    fi

    if [[ ! -f ${EXCLUSION_SST} ]]; then
      logCmd ${ANTSPATH}/antsApplyTransforms -d 3 -i ${EXCLUSION_MASK} \
        -o ${EXCLUSION_SST} \
        -r ${SINGLE_SUBJECT_TEMPLATE} \
        -t ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}/ExclusionToSST1Warp.nii.gz \
        -t ${OUTPUT_DIRECTORY_FOR_SINGLE_SUBJECT_TEMPLATE}/ExclusionToSST0GenericAffine.mat
      logCmd ${ANTSPATH}/ThresholdImage ${DIMENSION} ${EXCLUSION_SST} ${EXCLUSION_SST} 0.99 inf
    fi
  fi

  EXCLUSION_FLAG="-s ${EXCLUSION_SST} "
fi
